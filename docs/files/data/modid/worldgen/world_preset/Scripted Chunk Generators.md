...
* `world_traits` (optional, added in V4.3.3):

	**Before V4.4.0:** a map containing the traits this generator implements. The keys and values of this map are documented in bigglobe_world_traits_impl.

	**After V4.4.0:** world_traits is now the namespace and path of a file containing the traits. `modid:example` would load the traits from `/data/modid/worldgen/bigglobe_world_traits/example.json`. If multiple data packs provide this file, they will be merged at runtime. If multiple data packs provide the same trait within the file, the top data pack takes priority.
