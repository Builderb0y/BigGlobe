...
implements WaypointData {
	...
	public SyncedWaypointData sync() {
		return new SyncedWaypointData(this.id(), this.destination.entityId(), this.owner() != null, this.destinationPosition(), this.displayPosition, this.destination.name(), this.destination().color());
	}
}