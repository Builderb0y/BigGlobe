...
public class BedrockReplacer {
	...
	public static void generateBottom(SectionGenerationContext context) {
	...
	public static void generateTop(SectionGenerationContext context) {
		long sectionSeed = context.sectionSeed(0x733A5116222C3E7BL);
		int bedrockID = context.id(BlockStates.BEDROCK);
		PaletteStorage storage = context.storage();
		for (int index = 0; index < 4096; index++) {
			int y = index >>> 8;
			int chance = BigGlobeMath.squareI(y + 1);
			int rng = ((int)(Permuter.permute(sectionSeed, index))) & 255;
			if (rng < chance) {
				storage.set(index, bedrockID);
			}
