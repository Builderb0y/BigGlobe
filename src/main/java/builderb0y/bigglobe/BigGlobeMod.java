	...
	public static void regenWorlds(MinecraftServer server) {
		if (!server.getDefaultGameMode().isSurvivalLike()) {
			...
			server
				...
				.streamEntries()
				//only delete dimensions generated by big globe.
				.filter(options -> options.value().chunkGenerator() instanceof BigGlobeChunkGenerator || options.value().chunkGenerator() instanceof BigGlobeScriptedChunkGenerator)
				.peek(options -> LOGGER.info("Found " + MODNAME + " dimension " + UnregisteredObjectException.getKey(options)))
