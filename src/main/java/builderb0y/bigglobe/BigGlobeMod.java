	...
	public static void regenWorlds(MinecraftServer server) {
		if (!server.getDefaultGameMode().isSurvivalLike()) {
			...
			server
				...
				.streamEntries()
				//only delete dimensions generated by big globe.
				.filter((RegistryEntry<DimensionOptions> options) -> options.value().chunkGenerator() instanceof BigGlobeChunkGenerator || options.value().chunkGenerator() instanceof BigGlobeScriptedChunkGenerator)
