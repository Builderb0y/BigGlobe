...
public class OverworldBiomeLayout {
	...
	public static class Holder {
		...
		public final Registry<OverworldBiomeLayout> registry;
		public final transient OverworldBiomeLayout root;
		public final transient Set<ColumnValue<?>> usedValues;

		public Holder(Registry<OverworldBiomeLayout> registry) {
			...
			registry.streamEntries().sequential().forEachOrdered(entry -> {
				...
				if (key == ROOT_KEY) {
					...
					OverworldBiomeLayout parent = registry.getOrThrow(RegistryKey.of(BigGlobeDynamicRegistries.OVERWORLD_BIOME_LAYOUT_REGISTRY_KEY, new Identifier(parentName)));
					if (negated) {
			...
			this.root = registry.getOrThrow(ROOT_KEY);
	...
	public static class Printer {
		...
		*/
		...
		public static Printer parse(Registry<OverworldBiomeLayout> registry) {
			...
			Printer root = map.get(registry.getOrThrow(ROOT_KEY));
