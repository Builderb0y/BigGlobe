...
public class DoubleBetweenInsnTree extends BetweenInsnTree {
	...
	public static InsnTree create(
		...
		return new DoubleBetweenInsnTree(
			value.cast(parser, TypeInfos.DOUBLE, CastMode.IMPLICIT_THROW),
			min.cast(parser, TypeInfos.DOUBLE, CastMode.IMPLICIT_THROW),
			minInclusive,
			max.cast(parser, TypeInfos.DOUBLE, CastMode.IMPLICIT_THROW),
			maxInclusive
		);
	}
