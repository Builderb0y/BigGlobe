...
public class ForDoubleRangeInsnTree extends AbstractForRangeInsnTree {

	public ForDoubleRangeInsnTree(
		String loopName,
		...
		if (this.ascending) {
			...
			if (this.lowerBoundInclusive) {
				...
				if (this.upperBoundInclusive) {
					DoubleCompareConditionTree.lessThanOrEqual(load(this.variable.variable), upperBound).emitBytecode(method, null, mainLoop.end.getLabel());
				}
				else {
					DoubleCompareConditionTree.lessThan(load(this.variable.variable), upperBound).emitBytecode(method, null, mainLoop.end.getLabel());
				}
				this.body.emitBytecode(method);
				method.node.instructions.add(continuePoint);
				store(this.variable.variable, new AddInsnTree(load(this.variable.variable), step, DADD)).emitBytecode(method);
			...
			if (this.upperBoundInclusive) {
				...
				method.node.visitLabel(start);
				if (this.lowerBoundInclusive) {
					DoubleCompareConditionTree.greaterThanOrEqual(load(this.variable.variable), lowerBound).emitBytecode(method, null, mainLoop.end.getLabel());
				}
				else {
					DoubleCompareConditionTree.greaterThan(load(this.variable.variable), lowerBound).emitBytecode(method, null, mainLoop.end.getLabel());
				}
				this.body.emitBytecode(method);
				method.node.instructions.add(continuePoint);
				store(this.variable.variable, new SubtractInsnTree(load(this.variable.variable), step, DSUB)).emitBytecode(method);
				method.node.visitJumpInsn(GOTO, start);
			}
			else {
				method.node.instructions.add(continuePoint);
