...
public class TemplateScriptParser<I> extends ScriptParser<I> {
	...
	public InsnTree parseEntireInput() throws ScriptParsingException {
		...
		for (RequiredInput input : this.inputs.template.inputs) {
			...
			Map<String, List<FunctionHandler>> functions = parserCopy.environment.mutable().functions;
			functions.put("return", Collections.singletonList(handler));
			functions.put("throw",  Collections.singletonList(handler));
