...
public class UnaryInsnTreeTest extends OperatorTest {
	...
	public void testInt() throws ScriptParsingException {
		...
		for (String operator : INT_OPERATORS) {
			...
			IntUnaryOperator var = new ScriptParser<>(IntUnaryOperator.class, operator + " x").addEnvironment(new MutableScriptEnvironment().addVariableLoad("x", 1, TypeInfos.INT)).parse();
	...
	public void testLong() throws ScriptParsingException {
		...
		for (String operator : LONG_OPERATORS) {
			System.out.println(operator);
			LongUnaryOperator var = new ScriptParser<>(LongUnaryOperator.class, operator + " x").addEnvironment(new MutableScriptEnvironment().addVariableLoad("x", 1, TypeInfos.LONG)).parse();
	...
	public void testFloat() throws ScriptParsingException {
		...
		for (String operator : FLOAT_OPERATORS) {
			System.out.println(operator);
			FloatUnaryOperator var = new ScriptParser<>(FloatUnaryOperator.class, operator + "x").addEnvironment(new MutableScriptEnvironment().addVariableLoad("x", 1, TypeInfos.FLOAT)).parse();
	...
	public void testDouble() throws ScriptParsingException {
		...
		for (String operator : DOUBLE_OPERATORS) {
			System.out.println(operator);
			DoubleUnaryOperator var = new ScriptParser<>(DoubleUnaryOperator.class, operator + "x").addEnvironment(new MutableScriptEnvironment().addVariableLoad("x", 1, TypeInfos.DOUBLE)).parse();
