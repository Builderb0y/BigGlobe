...
import builderb0y.bigglobe.columns.scripted2.ColumnScript.ColumnYToDoubleScript;
...
public class ScriptedOreFeature extends AbstractOreFeature<ScriptedOreFeature.Config> {
	...
	public static class ScriptedOreBlockReplacer extends OreBlockReplacer {
		...
		public void replace(
			...
			BlockState oldState = context.palette().valueFor(context.storage().get(index));
			BlockState newState;
			synchronized (column) {
			...
			if (newState != null && newState != oldState) {
				int id = context.palette().idFor(newState, context.container());
