...
import builderb0y.bigglobe.columns.scripted2.compile.VoronoiImplCompileContext;
import builderb0y.bigglobe.columns.scripted2.types.VoronoiColumnValueType;
...
public class VoronoiColumnEntry extends AbstractColumnEntry {
	...
	public void emitFieldGetterAndSetter(ColumnEntryMemory memory, DataCompileContext context) {
		...
		for (Holder<VoronoiSettings> voronoiEntry : voronoiManager.getOptionsFor(this)) {
			...
			for (Map.Entry<String, Holder<ColumnEntry>> export : voronoiEntry.value().exports().entrySet()) {
