...
public class AndConditionTree implements ConditionTree {
	...
	public static ConditionTree create(ConditionTree left, ConditionTree right) {
		if (left instanceof ConstantConditionTree leftConstant) {
			...
			if (right instanceof ConstantConditionTree rightConstant && rightConstant.value) {
				//variable && true is safe to simplify.
				return left;
			}
			else {
				//don't short-circuit if the right condition is constant false.
